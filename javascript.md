...
2.[インデントはタブで4文字分](#インデントはタブで4文字分)
3.[変数名はローワーキャメル形式で宣言](変数名はローワーキャメル形式で宣言)
4.[数字や文字列の比較では厳密等価演算子を使う](数字や文字列の比較では厳密等価演算子を使う)
5.[for文で、配列をループさせる場合は最初に配列の長さを変数に格納しておく](for文で、配列を使う場合は最初に配列の長さを変数に格納しておく)
6.[オブジェクトの指定方法](#オブジェクトの指定方法)
7.[jQuery](#jQuery)
